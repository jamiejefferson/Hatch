...
const api: HatchApi = {
  ...
  moveLink: (id, folder) => ipcRenderer.invoke('links:move', id, folder),
  renameLinkFolder: (from, to) => ipcRenderer.invoke('links:rename-folder', from, to),
  savedCanvases: () => ipcRenderer.invoke('canvases:list'),
  saveCanvas: (saved) => ipcRenderer.invoke('canvases:save', saved),
  removeSavedCanvas: (id) => ipcRenderer.invoke('canvases:remove', id),
